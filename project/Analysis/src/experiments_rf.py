...
algorithm = 'RF'
runtime = 'Hyperparameter optimization in RF with Depth = range(80,100), and ntrees = 34 default parameters. All features used.\n \
Validation set has 400 points.'

log_name = 'RF_'+str(datetime.datetime.now()).replace(' ','_')\
...
depth_range = range(80,100)

for depth in tqdm(depth_range):
    classifier = RandomForest(ntrees = 34,depth = depth)
    ...
    logging.info('Depth = {} \t Train F1 = {} \t Validation F1 = {}'.format(depth,tf1,vf1))

logging.info("Time to run : {} s.".format(time()-start))

plt.plot(depth_range,train_f1,label='Train F1 Scores')
plt.plot(depth_range,val_f1,label='Validation F1 Scores')
plt.xlabel("Depth")
plt.ylabel("F1 Score")
plt.legend()
plt.grid()
